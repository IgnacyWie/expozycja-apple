...
function LoginPage({ initialMode = "login" }: AnimatedLoginProps) {
    ...
    if (mode === "login") {
      ...
      if (!email || !password || !confirmPassword) {
        ...
        if (signUpError) {
          ...
          console.log("✅ Account created via Supabase");
    ...
    <div className="relative grid min-h-screen lg:grid-cols-2">
      {successMessage && (
        <div className="pointer-events-none fixed inset-x-0 top-4 z-40 flex justify-center">
          <div className="pointer-events-auto rounded-full bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-emerald-900/40">
            {successMessage}
          </div>
        </div>
      )}
      <div className="pointer-events-none absolute right-4 top-4 z-30 flex">
        <Button
          asChild
          size="icon"
          variant="outline"
          className="pointer-events-auto h-9 w-9 rounded-full border-border/70 bg-background/80 backdrop-blur"
        >
          <Link href="/">
            <span className="sr-only">Close and go back home</span>
            <X className="h-4 w-4" />
          </Link>
        </Button>
      </div>
      <div className="relative hidden flex-col justify-between bg-gradient-to-br from-primary/90 via-primary to-primary/80 p-12 text-primary-foreground lg:flex">
        <div className="relative z-20">
          <div className="flex items-center gap-2 text-lg font-semibold tracking-tight">
            ...
            <span className="text-primary-foreground">Mant</span>
        ...
        <div className="bg-grid-white/[0.05] bg-[size:20px_20px] absolute inset-0" />
      ...
      <div className="flex items-center justify-center bg-background p-8">
        <div className="w-full max-w-[420px]">
          <div className="mb-12 flex items-center justify-center gap-2 text-lg font-semibold tracking-tight lg:hidden">
            <div className="flex size-8 items-center justify-center rounded-lg bg-primary/10">
              <Sparkles className="size-4 text-primary" />
            </div>
            <span className="text-zinc-950 dark:text-zinc-50">Mant</span>
